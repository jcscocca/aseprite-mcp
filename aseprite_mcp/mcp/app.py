...
server = FastMCP(
    ...
    instructions=(
        ...
        "  6. Animate: set_frame_duration() for timing, copy_cel() to re-pose "
        "(copy a frame's pixels, then edit the copy), delete_frame() to drop "
        "one, add_tag() to name frame ranges as animations\n"
        "  7. export() — png, animated gif, or spritesheet + Godot-ready JSON "
        "metadata (tags become named animations)\n\n"
        ...
        "Erasing: clear_region() erases rectangles; draw_pixels with "
        "'#00000000' erases single pixels (raw pixel set); draw_shape cannot "
        "erase (tool strokes alpha-blend)."
    ),
)
